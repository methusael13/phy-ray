...
#ifndef PHYRAY_OPTIMIZE
    Bounds3(const Bounds3<T>& ref) {
        ASSERT(!ref.pMin.hasNaNs() && !ref.pMax.hasNaNs());
        pMin = ref.pMin; pMax = ref.pMax;
    }

    Bounds3<T>& operator=(const Bounds3<T>& rhs) {
        ASSERT(!rhs.pMin.hasNaNs() && !rhs.pMax.hasNaNs());
